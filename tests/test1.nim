...
suite "test server":
  ...
  # start jester server
  let
    p = startProcess("." / AppName, args = @["--appname", AppName])
    db = openDb()
    tuser = "test_user"
    tpass = "test_pass"
  ...
  test "http userconf page":
    ...
    res = c.request(Host & "userconf", headers = h)
    check not res.body.contains("<table>")
  ...
  test "http userconf api":
    ...
    let jdata = %*{muser: {"permission": pmMember.ord, "enabled": false}}
    c.headers.clear
    res = c.request(Host & "userconf", HttpPost, body = $jdata)
    check res.body.parseJson["result"].getBool
